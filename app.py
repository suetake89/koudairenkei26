    ...
    if has_motivation:
        execution_notes.append((
            ...
            [
                ...
                (rf"0 <= {w}\quad({idx})", "評価値は負にならないようにする。"),
                (
                    rf"{w} <= {m}\quad({idx})",
        ...
        execution_notes.append((
            ...
            [
                (
                    rf"{next_m} <= \min\{{100,\ {m}-a{z}+b{r_var}+{g}\}}",
                    "PuLPではこのminを直接使わず、以下の2つの上限制約に分割する。",
                ),
                (rf"{next_m} <= 100", "1つ目の上限制約。モチベーションの上限を100にする。"),
                (
                    rf"{next_m} <= {m}-a{z}+b{r_var}+{g}",
